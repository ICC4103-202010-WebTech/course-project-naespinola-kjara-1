class DatesToVotesController < ApplicationController
  ...
  def index
    @dates_to_votes = DatesToVote.joins(event: :user).joins(:votes).where("users.id= #{current_person.id}")
  end
  ...
  private
    ...
    def dates_to_vote_params
      params.fetch(:dates_to_vote, {}).permit(:date, :event_id)
    end
end
